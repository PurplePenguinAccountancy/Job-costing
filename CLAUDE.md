...
## 19. Authentication — Auth.js v5, email magic-link (Addendum 2.M), live-verified

No password ever handled by this app. Built per the already-decided architecture in 2.M and live-tested end-to-end: sign in → session → tenant list scoped to membership → per-tenant access gate → sign out → route re-protected.

- **Adapter tables** (`schema/auth.ts`): `user`/`account`/`session`/`verificationToken`, matching `@auth/drizzle-adapter`'s expected Postgres schema exactly, deliberately kept separate from the domain `users` table rather than reshaping it — no RLS (infrastructure, not tenant data, same reasoning as `tenants`/`users`). `auth.ts`'s `session` callback resolves `session.user.id` to the real `users.id` by email (creating the row if it's a first sign-in), so every existing `createdBy`/`ownerId`/`approvedBy` foreign key keeps working with zero changes elsewhere.
- **Route protection**: no `middleware.ts` — Next.js middleware runs on the Edge runtime by default, which can't load the Postgres-backed adapter (drags in Node-only modules like `stream`). The real gate is `src/app/t/[tenantId]/layout.tsx`, a normal Node.js server component: confirms a session exists, then checks `tenant_memberships` for that specific user+tenant (not just "is anyone signed in") — live-verified both ways, a member sees the dashboard, a non-member gets a clear "doesn't have access" page, and either way it's still exactly one choke point since every nested route sits under this layout.
- **Homepage** now requires sign-in and lists only the tenants the signed-in user actually belongs to (`getUserTenants`), not every tenant in the database.
- **Email delivery**: SMTP (`smtp.ionos.co.uk`) is the configured path, but outbound port 587 is **confirmed blocked at the network level in this dev environment** (a direct OS-level TCP test times out completely — not an app bug, not a credentials issue). Real delivery is deferred per explicit instruction ("fix this later"). In the meantime, `sendVerificationRequest` in `auth.ts` attempts real SMTP first and, on failure, logs the actual sign-in link to the server console instead of hard-failing the request — this is what made live end-to-end testing possible without working SMTP, and doubles as a reasonable dev-mode fallback going forward. **Before any real pilot use**: either unblock port 587 from wherever this actually runs, or switch delivery to an HTTP-based transactional email API (Resend recommended — same port-443 pattern already proven reliable for Xero/Azure/this app's other integrations) so delivery never depends on SMTP ports at all.
- **A real RLS robustness gap, found via live testing and fixed everywhere, not just here**: `rls-helpers.ts`'s shared tenant-isolation policy (used by all 15 tenant-scoped tables) and `memberships.ts`'s policies both did `current_setting(..., true)::uuid` directly — a malformed (e.g. empty-string) session variable throws `invalid input syntax for type uuid` and 500s the whole query, instead of the policy just evaluating to "no match" the way a security check should degrade. Observed live: under the dev server's request patterns, `app.current_user_id` was seen reading back as `''` instead of `NULL` on an already-used pooled connection at the start of a fresh transaction — not fully root-caused (bounded investigation: confirmed via `pg_backend_pid()` it was the same physical connection each time, ruled out true concurrent cross-talk between different connections), but fixed at both ends regardless: `withTenant` now sets a real SQL `NULL` instead of `userId ?? ""` when there's no user, and every policy wraps the cast in `NULLIF(current_setting(...), '')` so a malformed value can never produce a hard error, only a clean deny. Migration `0008_material_rogue.sql`. Worth a closer root-cause pass before real concurrent multi-user production traffic, even though the defensive fix already closes the crash (and the cross-tenant-leak risk a naive fix might have missed).
- **Seeded test accounts**: `Alex@purplepenguinaccountancy.co.uk` (real account, for actual personal use) and `invoices@wayleavejc.co.uk` (the already-wired-up IMAP mailbox, used specifically so sign-in could be tested live end-to-end by fetching the real magic-link email via IMAP rather than depending on a human checking their own inbox) — both seeded as editors on the demo tenant.
