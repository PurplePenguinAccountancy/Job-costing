...
## 22. Demoable UI pass — basic app shell, not a redesign

Requested explicitly: a UI good enough to demo to the end client and let them start using it, on the understanding it can be completely redesigned later. This is a shell/polish pass, not a visual redesign — no page's actual content, tables, or forms changed shape.

- **Persistent sidebar** (`src/app/t/[tenantId]/AppShell.tsx` + `app-shell.module.css`), wired into the existing `layout.tsx` access-control gate: brand mark, current tenant name + role, links to every top-level section (Dashboard/Capture/Approvals/Labour/Billing-WIP/Team/Xero), sign-out. Replaces each page's own duplicated "← Dashboard" back-link and ad-hoc nav-links row — one persistent nav instead of N reinvented ones. No active-page highlighting (would need client-side pathname tracking, which this codebase deliberately avoids per its zero-client-JS pattern for form pages) — an acceptable gap for a first pass, not a redesign blocker.
- **Global design tokens** (`globals.css`): brand/neutral CSS custom properties, and the body's `font-family` now actually references the already-loaded Geist font variable (it was previously hardcoded to `Arial, Helvetica, sans-serif`, silently overriding the `next/font` setup — a real, if harmless, pre-existing oversight fixed in passing).
- **Shared brand mark** (`src/app/BrandMark.tsx`, styled via global classes since it crosses CSS-module boundaries) on every sign-in/setup-account screen and the home tenant-list page, plus real card elevation on the auth flow's `.card` (previously borderless, floating on plain white).
- **Root metadata and home-page copy** changed from "internal build preview / not the product UI" to real product framing — this was the literal, explicit signal that the existing UI had never been intended as client-facing.
- **A real, unrelated bug caught while verifying, not a UI issue**: while re-testing the team page for this pass, checked the actual `tenant_memberships` rows directly and found Alex and Sam Rivera were still `viewer` — the earlier §20 role-restoration (done immediately post-compaction, before the "check the DB, not just the DOM" lesson from §21's settings-save bug) had suffered the exact same failure mode: a `read_page` snapshot showing "Editor" selected was actually just the locally-set `<select>` value from `form_input`, never confirmed against a real server round-trip. Re-fixed and this time verified against the database directly, not the rendered page. **A second, session-specific cause also surfaced**: the Browser pane was intermittently not visually composited (`screenshot` failing with "the Browser pane is not displayed"), during which pixel-coordinate `computer` clicks silently failed to submit the form at all, while `read_page`/`form_input` (DOM/accessibility-tree based, not compositing-dependent) kept working — producing exactly this kind of false-positive "it worked" signal. Worked around by driving the form via `javascript_tool` (`element.closest("form").requestSubmit()`) instead of coordinate clicks, which doesn't depend on the pane being visually rendered.
- **Explicitly out of scope for this pass** (by design, not oversight): no visual redesign of any table/form, no client-side active-nav-link highlighting, no mobile-specific testing beyond a basic CSS breakpoint on the sidebar, no retrofit of every page's already-consistent hardcoded hex colors onto the new CSS variables (new code uses the variables; old code is untouched to keep the diff reviewable).
